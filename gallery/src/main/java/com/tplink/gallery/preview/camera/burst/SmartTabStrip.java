...
import com.android.gallery3d.util.SelectBurstHelper;
...
class SmartTabStrip extends LinearLayout {
    ...
    SmartTabStrip(Context context, AttributeSet attrs) {
        ...
        indicatorAlwaysInCenter = a
                .getBoolean(R.styleable.stl_SmartTabLayout_stl_indicatorAlwaysInCenter,
                        indicatorAlwaysInCenter);
        indicatorWithoutPadding = a
                .getBoolean(R.styleable.stl_SmartTabLayout_stl_indicatorWithoutPadding,
                        indicatorWithoutPadding);
        indicatorInFront = a.getBoolean(R.styleable.stl_SmartTabLayout_stl_indicatorInFront,
                indicatorInFront);
        indicationInterpolatorId = a
                .getInt(R.styleable.stl_SmartTabLayout_stl_indicatorInterpolation,
                        indicationInterpolatorId);
        indicatorGravity = a.getInt(R.styleable.stl_SmartTabLayout_stl_indicatorGravity,
                indicatorGravity);
        indicatorColor = a.getColor(R.styleable.stl_SmartTabLayout_stl_indicatorColor,
                indicatorColor);
        indicatorColorsId = a.getResourceId(R.styleable.stl_SmartTabLayout_stl_indicatorColors,
                indicatorColorsId);
        indicatorThickness = a.getDimensionPixelSize(
                R.styleable.stl_SmartTabLayout_stl_indicatorThickness, indicatorThickness);
        indicatorWidth = a.getLayoutDimension(R.styleable.stl_SmartTabLayout_stl_indicatorWidth,
                indicatorWidth);
        ...
        underlineColor = a.getColor(R.styleable.stl_SmartTabLayout_stl_underlineColor,
                underlineColor);
        ...
        dividerColorsId = a.getResourceId(R.styleable.stl_SmartTabLayout_stl_dividerColors,
                dividerColorsId);
        ...
        final int[] indicatorColors = (indicatorColorsId == NO_ID) ? new int[] {
                indicatorColor
        } : getResources().getIntArray(indicatorColorsId);

        final int[] dividerColors = (dividerColorsId == NO_ID) ? new int[] {
                dividerColor
    ...
    private void drawDecoration(Canvas canvas) {
        ...
        final boolean isLayoutRtl = SelectBurstHelper.isLayoutRtl(this);
        ...
        if (tabCount > 0) {
            ...
            int selectedStart = SelectBurstHelper.getStart(selectedTab, indicatorWithoutPadding);
            int selectedEnd = SelectBurstHelper.getEnd(selectedTab, indicatorWithoutPadding);
            int left;
            int right;
            if (isLayoutRtl) {
                left = selectedEnd;
            ...
            if (selectionOffset > 0f && selectedPosition < (getChildCount() - 1)) {
                ...
                int nextStart = SelectBurstHelper.getStart(nextTab, indicatorWithoutPadding);
                int nextEnd = SelectBurstHelper.getEnd(nextTab, indicatorWithoutPadding);
                if (isLayoutRtl) {
            ...
            drawIndicator(canvas, left, right, height);
    ...
    private void drawSeparator(Canvas canvas, int height, int tabCount) {
        ...
        for (int i = 0; i < tabCount - 1; i++) {
            ...
            int end = SelectBurstHelper.getEnd(child);
            int endMargin = SelectBurstHelper.getMarginEnd(child);
    ...
    private void drawIndicator(Canvas canvas, int left, int right, int height) {
        ...
        Path path = new Path();
        path.moveTo(center - width / 2, margin);// 此点为多边形的起点
        path.lineTo(center + width / 2, margin);
        path.lineTo(center, width - margin);
        path.close(); // 使这些点构成封闭的多边形
        canvas.drawPath(path, paint);

        // 画底部的三角形
        Path path2 = new Path();
        path2.moveTo(center - width / 2, getHeight() - margin);// 此点为多边形的起点
        path2.lineTo(center + width / 2, getHeight() - margin);
        path2.lineTo(center, getHeight() - width + margin);
    ...
    private void drawIndicator(Canvas canvas, int left, int right, int height, float thickness,
        ...
        if (indicatorCornerRadius > 0f) {
            canvas.drawRoundRect(indicatorRectF, indicatorCornerRadius, indicatorCornerRadius,
                    indicatorPaint);
        } else {
