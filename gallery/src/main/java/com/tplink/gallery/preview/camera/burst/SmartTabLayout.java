...
public class SmartTabLayout extends HorizontalScrollView {
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        if (tabStrip.isIndicatorAlwaysInCenter() && tabStrip.getChildCount() > 0) {
            ...
            int start = (w - SelectBurstHelper.getMeasuredWidth(firstTab)) / 2 - SelectBurstHelper.getMarginStart(firstTab);
            int end = (w - SelectBurstHelper.getMeasuredWidth(lastTab)) / 2 - SelectBurstHelper.getMarginEnd(lastTab);
    ...
    private void scrollToTab(int tabIndex, float positionOffset) {
        ...
        int widthPlusMargin = SelectBurstHelper.getWidth(selectedTab) + SelectBurstHelper.getMarginHorizontally(selectedTab);
        ...
        if (tabStrip.isIndicatorAlwaysInCenter()) {
            ...
            if (0f < positionOffset && positionOffset < 1f) {
                ...
                int selectHalfWidth = SelectBurstHelper.getWidth(selectedTab) / 2 + SelectBurstHelper.getMarginEnd(selectedTab);
                int nextHalfWidth = SelectBurstHelper.getWidth(nextTab) / 2 + SelectBurstHelper.getMarginStart(nextTab);
            ...
            int x;
            if (isLayoutRtl) {
                int first = SelectBurstHelper.getWidth(firstTab) + SelectBurstHelper.getMarginEnd(firstTab);
                int selected = SelectBurstHelper.getWidth(selectedTab) + SelectBurstHelper.getMarginEnd(selectedTab);
                x = SelectBurstHelper.getEnd(selectedTab) - SelectBurstHelper.getMarginEnd(selectedTab) - extraOffset;
                x -= (first - selected) / 2;
            } else {
                int first = SelectBurstHelper.getWidth(firstTab) + SelectBurstHelper.getMarginStart(firstTab);
                int selected = SelectBurstHelper.getWidth(selectedTab) + SelectBurstHelper.getMarginStart(selectedTab);
                x = SelectBurstHelper.getStart(selectedTab) - SelectBurstHelper.getMarginStart(selectedTab) + extraOffset;
                x -= (first - selected) / 2;
            }
        ...
        if (titleOffset == TITLE_OFFSET_AUTO_CENTER) {
            ...
            if (isLayoutRtl) {
                x = -SelectBurstHelper.getWidthWithMargin(selectedTab) / 2 + getWidth() / 2;
                x -= SelectBurstHelper.getPaddingStart(this);
            } else {
                x = SelectBurstHelper.getWidthWithMargin(selectedTab) / 2 - getWidth() / 2;
                x += SelectBurstHelper.getPaddingStart(this);
            }
        ...
        int start = SelectBurstHelper.getStart(selectedTab);
        int startMargin = SelectBurstHelper.getMarginStart(selectedTab);
        if (isLayoutRtl) {
            x += start + startMargin - extraOffset - getWidth() + SelectBurstHelper.getPaddingHorizontally(this);
        } else {
