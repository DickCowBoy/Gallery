...
public class ExifTag {
    ...
    public boolean setValue(String value) {
        ...
        byte[] finalBuf = buf;
        if (buf.length > 0) {
            /// M: [BUG.MODIFY] get find the first index of '0'. and copy to new byte[] @{
            ...
            // if index == buf.length, means there is not has '0' in buf. so should add '0' for finalBuf(index +1).
