...
public class TiledTexture implements Texture {
    ...
    public static class Uploader implements OnGLIdleListener {
        ...
        public synchronized boolean contains(TiledTexture t){
            if(mTextures.contains(t)){
                return true;
            }
            return false;
        }
    ...
    private static class Tile extends UploadedTexture {
        ...
        protected Bitmap onGetBitmap() {
            ...
            //freeResources 会对sCanvas 释放    解决GL和UI线程不同步的问题
            try {
                Bitmap localBitmapRef = bitmap;
//            bitmap = null;

                if (localBitmapRef != null && !localBitmapRef.isRecycled()) {
            ...
            return sUploadBitmap == null ?
                    Bitmap.createBitmap(TILE_SIZE, TILE_SIZE, Config.ARGB_8888) : sUploadBitmap;
        }
    ...
    public boolean isValide(){
        ...
        if(n != 0) {
            for (int i = 0; i <= n / 2; i++) {
                if (!checkTile(mTiles[i]) || !checkTile(mTiles[n - 1 - i])){
                    return false;
                }
            }
        }
    ...
    /**
     * 释放画布等共有资源，一般只能用于onPause()生命周期，否则会影响其他页面，出现一片黑的缩略图。
     */
