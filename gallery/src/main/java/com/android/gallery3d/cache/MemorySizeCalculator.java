...
public class MemorySizeCalculator {
    ...
    static final int MAX_MEMORY = 100 * 1024 * 1024;  //最大内存
    ...
    private static MemorySizeCalculator mInstance;
    ...
    private MemorySizeCalculator(Context context) {
        this(context,
                (ActivityManager) context.getSystemService(Context.ACTIVITY_SERVICE),
                new DisplayMetricsScreenDimensions(context.getResources().getDisplayMetrics()));
    }

    public static MemorySizeCalculator get(Context context){
        if(mInstance == null){
            synchronized(MemorySizeCalculator.class){
                if(mInstance == null){
                    mInstance = new MemorySizeCalculator(context);
                }
            }
        }
        return mInstance;
    ...
    MemorySizeCalculator(Context context, ActivityManager activityManager, ScreenDimensions screenDimensions) {
        ...
        int targetPoolSize = GalleryBitmapPool.CAPACITY_BYTES;
        int targetMemoryCacheSize = MAX_MEMORY - targetPoolSize;
        ...
        Log.i(TAG, "Calculated memory cache size: " + toMb(memoryCacheSize) + " pool size: " + toMb(bitmapPoolSize)
