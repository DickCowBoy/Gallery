...
public class PhotoFallbackEffect extends Animation {
    ...
    public static class Entry {
        public int index;
        public MediaBean path;
        public Rect source;
        public Rect dest;
        public RawTexture texture;
        public int rotation;

        public Entry(MediaBean path, Rect source, RawTexture texture, int rotation) {
            this.path = path;
            this.source = source;
    ...
    public interface PositionProvider {
        ...
        public int getItemIndex(MediaBean path);
    }
    ...
    public void addEntry(MediaBean path, Rect rect, RawTexture texture, int rotation) {
        mList.add(new Entry(path, rect, texture, rotation));
    }

    public Entry getEntry(MediaBean path) {
    ...
    private void drawEntry(GLCanvas canvas, Entry entry) {
        ...
        if (d.width() == d.height()) {
            ...
            // 如果不是正方形需要做特别处理
            float fullScale = Math.max(((float)d.width()) / s.width(),((float)d.height() / s.height()));
            boolean hCut = Float.compare(((float)d.width()) / s.width(), ((float)d.height() / s.height())) < 0;
            float scale = fullScale * p + 1 * (1 - p);
            ...
            if (hCut) {
                float cutDy = (s.width() - d.width() / fullScale) / 2;
                float oriCutDy = (cutDy * (h / 1.0F / s.height()));
                cutDy = cutDy * scale;
                float ch = s.height() * scale;
                float cw = s.width() * scale;
                // draw the center part
                mTarget.set(cx - cw / 2  + cutDy, cy - ch / 2, cx + cw / 2  - cutDy, cy + ch / 2);
                mSource.set(oriCutDy, 0, w - oriCutDy, h);
                canvas.drawTexture(entry.texture, mSource, mTarget);
                ...
                // draw the upper part
                mTarget.set(cx - cw / 2, cy - ch / 2, cx + cw / 2 + cutDy, cy - ch / 2);
                mSource.set(0, 0, oriCutDy, h);
                canvas.drawTexture(entry.texture, mSource, mTarget);
                // draw the bottom part
                mTarget.set(cx - cw / 2 - cutDy, cy + ch / 2, cx + cw / 2, cy + ch / 2);
                mSource.set(w- oriCutDy, 0, w, h);
                canvas.drawTexture(entry.texture, mSource, mTarget);

                canvas.restore();
            } else {
                float cutDx = (s.height() - d.height() / fullScale) / 2;
                float oriCutDx = (cutDx * (h / 1.0F / s.height()));
                cutDx = cutDx * scale;
                float ch = s.height() * scale;
                float cw = s.width() * scale;
                // draw the center part
                mTarget.set(cx - cw / 2, cy - ch / 2 + cutDx, cx + cw / 2, cy + ch / 2 - cutDx);
                mSource.set(0, oriCutDx, w, h - oriCutDx);
                canvas.drawTexture(entry.texture, mSource, mTarget);
                ...
                // draw the upper part
                mTarget.set(cx - cw / 2, cy - ch / 2, cx + cw / 2, cy - ch / 2 + cutDx);
                mSource.set(0, 0, w, oriCutDx);
                canvas.drawTexture(entry.texture, mSource, mTarget);
                // draw the bottom part
                mTarget.set(cx - cw / 2, cy + ch / 2 - cutDx, cx + cw / 2, cy + ch / 2);
                mSource.set(0, h - oriCutDx, w, h);
                canvas.drawTexture(entry.texture, mSource, mTarget);
