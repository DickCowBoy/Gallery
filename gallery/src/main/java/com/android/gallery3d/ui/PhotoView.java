...
import com.android.gallery3d.app.SynchronizedHandler;
...
import com.android.gallery3d.util.DimenUtils;
...
public class PhotoView extends GLView {
    ...
    public PhotoFallbackEffect mEffect;
    boolean mImageChanged = false;
    boolean mHasJoin = false;
    ...
    public interface Model extends TileImageView.TileSource {
        ...
        // Returns true if the item is a Burst Photo.
        public boolean isBurstPhoto(int offset);

        // Returns true if the item is a Gif Image
    ...
    private boolean mIsScalingSupported = true;
    ...
    // 是否是其他应用调用图库进行多选
    private boolean mIsMultiSelectPreview = false;

    private GLHost mGLHost;

    public PhotoView(GLHost glHost, boolean canSwitchMode) {
        mTileView = new TileImageView(glHost);
        addComponent(mTileView);
        this.mGLHost = glHost;
        mPlaceholderColor = mGLHost.getContext().getResources().getColor(
                R.color.photo_placeholder);
        mEdgeView = new EdgeView(mGLHost.getContext());
        addComponent(mEdgeView);
        mUndoBar = new UndoBarView(mGLHost.getContext());
        marginBottom = DimenUtils.dpToPixel(mGLHost.getContext(), 10);
        ...
        mNoThumbnailText = StringTexture.newInstance(mGLHost.getContext()
                        .getString(R.string.no_thumbnail), mGLHost.getContext().getResources()
                        .getDimensionPixelSize(R.dimen.albumset_title_font_size),
                Color.WHITE);
        /// @}

        mHandler = new MyHandler(glHost.getGLRoot());

        mGestureListener = new MyGestureListener();
        mGestureListener.setCanSwitchMode(canSwitchMode);
        mGestureRecognizer = new GestureRecognizer(mGLHost.getContext(), mGestureListener);

        mPositionController = new PositionController(mGLHost.getContext(),
        ...
        mVideoPlayIcon = new ResourceTexture(mGLHost.getContext(), R.drawable.ic_control_play);
        ...
        public void handleMessage(Message message) {
            switch (message.what) {
                ...
                case MSG_DELETE_ANIMATION_DONE: {
                    ...
                    mListener.onDeleteImage((MediaBean) message.obj, message.arg1);
                    ...
                    // Fix #19431，按UE要求，最后一张直接删除，不用显示撤销栏过几秒后再自动删除
    ...
    public void setIsMultiSelectPreview(boolean isMultiSelectPreview) {
        mIsMultiSelectPreview = isMultiSelectPreview;
    }
    ...
    public void notifyDataChange(int[] fromIndex, int prevBound, int nextBound) {
        ...
        Log.e(TAG, "fromIndex:" + Arrays.toString(fromIndex) +" prevBound:" + prevBound
                + " nextBound:" + nextBound);
        ...
        // Hide undo button if we are too far away Fix #40913 no need to control this
    ...
    protected void onLayout(
        ...
        Rect windowRect = getGLRoot().getWindowRect();
        mUndoBar.layout(0 + windowRect.left,
                h - mUndoBar.getMeasuredHeight() - marginBottom - windowRect.bottom,
                w - windowRect.right, h - marginBottom - windowRect.bottom);
    ...
    private int getPanoramaRotation() {
        ...
        int orientation = mGLHost.getContext().getResources().getConfiguration().orientation;
    ...
    class FullPicture implements Picture {
        ...
        public void reload() {
            ...
            mIsBurstPhoto = mModel.isBurstPhoto(0);
            mIsGifImage = mModel.isGifImage(0);
        ...
        public void draw(GLCanvas canvas, Rect r) {
            if(!mHasJoin && mListener != null){
                mHasJoin = true;
                mListener.onJoinTransparent();
            }
        ...
        private void drawTileView(GLCanvas canvas, Rect r) {
            ...
            // draw play icon when media item is video or gif
            // scaling is not supported for these media type
            if (mIsVideo || mIsGifImage) {
                drawVideoPlayIcon(canvas, s);
            }

            if (mIsVideo){
                // 视频不支持缩放
                setScalingSupport(false);
            } else {
                setScalingSupport(true);
            }
    ...
    private class ScreenNailPicture implements Picture {
        ...
        public void reload() {
            ...
            mIsBurstPhoto = mModel.isBurstPhoto(mIndex);
            mIsGifImage = mModel.isGifImage(mIndex);
    ...
    private void setScalingSupport(boolean is) {
        mIsScalingSupported = is;
    }
    ...
    private class MyGestureListener implements GestureRecognizer.Listener {
        ...
        public void setCanSwitchMode(boolean canSwitchMode) {
            this.mCanSwitchMode = canSwitchMode;
        }
        ...
        public boolean onSingleTapUp(float x, float y) {
            ...
            if (mFilmMode && !mDownInScrolling) {
                ...
                MediaBean item = mModel.getMediaItem(0);
                int supported = 0;
                if (item != null) supported = item.getSupportedOperations();
                if ((supported & MediaBean.SUPPORT_ACTION) == 0) {
                    setFilmMode(false);
                    mIgnoreUpEvent = true;
                    return true;
                }
        ...
        public boolean onDoubleTap(float x, float y) {
            ...
            if (!mIsScalingSupported) return true;
            ...
            // 根据UE要求，双击放大时缩放比变为默认缩放比的2.5倍
            // 如果默认缩放比的2.5倍大于最大限度的缩放比，则按照最大限度缩放比进行缩放
            float scaleLimit = controller.getImageScaleLimit();
            float targetScale = scale * 2.5f;
            if (controller.isAtMinimalScale()) {
                if (targetScale >= scaleLimit) {
                    controller.zoomIn(x, y, scaleLimit);
                } else {
                    controller.zoomIn(x, y, targetScale);
                }
            } else {
        ...
        private boolean flingImages(float velocityX, float velocityY, float dY) {
            ...
            int maxVelocity = DimenUtils.dpToPixel(mGLHost.getContext(), MAX_DISMISS_VELOCITY);
            int escapeVelocity = DimenUtils.dpToPixel(mGLHost.getContext(), SWIPE_ESCAPE_VELOCITY);
            int escapeDistance = DimenUtils.dpToPixel(mGLHost.getContext(), SWIPE_ESCAPE_DISTANCE);
        ...
        private void deleteAfterAnimation(int duration) {
            MediaBean item = mModel.getMediaItem(mTouchBoxIndex);
            if (item == null) return;
            mListener.onCommitDeleteImage();
        ...
        public boolean onScale(float focusX, float focusY, float scale) {
            if (!mIsScalingSupported && scale > 1 && !getFilmMode())  {
                return true;
            }
            if (mIgnoreSwipingGesture) return true;
            ...
            if (mCanSwitchMode && mCanChangeMode && largeEnough) {
        ...
        public void onDown(float x, float y) {
            ...
            if (mFilmMode) {
                ...
                if (mTouchBoxIndex < mPrevBound || mTouchBoxIndex > mNextBound) {
                    ...
                    // 如果是多选调用预览页面,则不支持上下滑动删除
                    if (mIsMultiSelectPreview) {
    ...
    public void setAnimationListener(Animation.AnimationListener mAnimationListener) {
    ...
    protected void render(GLCanvas canvas) {
        ...
        for (int i = neighbors; i >= -neighbors; i--) {
            ...
            // 当渲染的是屏幕中间的图片时, 需要生成缩小的动效。动效的生成必须通过GLCanvas来渲染得到,
            // 渲染是在GLThread线程中进行的, 直接在PhotoView中调用GLCanvas绘制动效是会报错的
            // 滑动开始的时候,需要FullPicture的reload方法清空原来的动效以便生成新的动效
            // 在滑动过程中,不能生成动效,因为render方法会调用许多次,并且此时FullPicture也不在屏幕正中间,生成的
            // 动效位置也不对。如果在滑动过程中不断生成动效会导致非常严重的UI线程阻塞问题
            // 滑动完成后,FullPicture会居中,render会被再次调用,此时再绘制新的动效并保存起来
            // 绘制完新动效后,需要将滑动标识mImageSwiped置为false,以便下一次滑动调用
            // 一旦新的动效生成后,除非触发FullPicture的reload方法,否则mEffect是不会被置为null的,
            ...
            if (i == 0 && screenNail instanceof TiledScreenNail) {
                ...
                if (mFirst || (mEffect == null
                    ...
                    if (mEffect != null) {
                        ...
                        //给动画加上监听
                        mEffect.setAnimationListener(mAnimationListener);
                    }
                    mImageChanged = false;
                }
            }
    ...
    private boolean slideToNextPicture() {
        if (!mModel.hasNext()) return false;
        switchToNextImage();
    ...
    private boolean slideToPrevPicture() {
        // Fix #45940 mPreBound 存在问题
        if (!mModel.hasPre()) return false;
        switchToPrevImage();
        mPositionController.startHorizontalSlide();
        return true;
    }
    ...
    // 渲染过程
    private PhotoFallbackEffect buildFallbackEffect(GLCanvas canvas, Rect rect) {
        //  创建新动效类
        ...
        ScreenNail sc = mTileView.getScreenNail();
        ...
        // 获取屏幕旋转标志，不能在这里做宽高交换，否则会导致无法修复的动效Bug
        // 需要在PhotoFallbackEffect里面进行修改
        int rotation = mModel.getImageRotation(0);
        // 进行材质渲染,并将渲染的结果放到到动效类中保存起来
        // 这里保存的就是缩小动画的开始材质, 当回退到AlbumPage时,会在onResume中取出(mResumEffect != null)
        if (rotation % 180 != 0) {
            int t = width;
            width = height;
            height = t;
        }
        RawTexture texture = new RawTexture(width, height, true);
        canvas.beginRenderTarget(texture);
        canvas.rotate(rotation, 0, 0, 1);
        // 使用Matrix计算旋转后需要画的位置
        Matrix m = new Matrix();
        m.postRotate(-rotation , 0 , 0);
        RectF rectF = new RectF(0 ,0 ,width, height);
        m.mapRect(rectF);
        sc.draw(canvas, (int)rectF.left , (int)rectF.top, sc.getWidth(), sc.getHeight());
        canvas.endRenderTarget();
        effect.addEntry(item, rect, texture, 0);
        return effect;
    }
}
