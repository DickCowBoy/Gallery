...
public class TiledScreenNail implements ScreenNail {
    ...
    private BitmapTexture mAnimationTexture;
    ...
    public void recycle() {
        ...
        if (mAnimationTexture != null) {
            mAnimationTexture.recycle();
            mAnimationTexture = null;
        }
    }
    ...
    public void draw(GLCanvas canvas, int x, int y, int width, int height) {
        if (mTexture == null || !mTexture.isReady()) {
            ...
            if(mDrawPlaceholder) {
                // 判断图片是否存在，如果存在则使用图片作为placeholder，
                // 不存在mBitmap对象则会使用默认颜色填充
                //　主要的作用是左右滑动过快的时候，图片还没有加载出来，需要填充背景默认颜色
                if (mBitmap != null) {
                    if (mAnimationTexture == null) {
                        mAnimationTexture = new BitmapTexture(mBitmap);
                    }
                    canvas.drawTexture(mAnimationTexture, x, y, width, height);
                } else {
        ...
        // 删除判断isAnimating的过程，前面已经使用了图片进行填充
        // 这里如果在动画过程再去填充背景颜色，就会造成放大过程会闪的情况
