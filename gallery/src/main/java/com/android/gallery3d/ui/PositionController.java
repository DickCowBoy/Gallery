...
class PositionController {
    ...
    public static final int SNAPBACK_ANIMATION_TIME = 300;
    ...
    public static final float SCALE_LIMIT = 5;
    ...
    public void setImageSize(int index, Size s, Rect cFrame) {
        ...
        // 这里删除了needupdate标记,因为该标志导致了切换幻灯片模式时,
        // 同等尺寸的照片右边图片缩放比例不一致的情况
        setBoxSize(index, s.width, s.height, false);
        updateScaleAndGapLimit();
        snapAndRedraw();
    }
    ...
    public void moveBox(int fromIndex[], boolean hasPrev, boolean hasNext,
        ...
        RangeArray<Box> boxesBackup = new RangeArray<>(-BOX_MAX, BOX_MAX);
        /// @}
    ...
    public float getImageScaleLimit() {
        Box b = mBoxes.get(0);
        return b.mScaleMax;
    }
    ...
    private float getMinimalScale(Box b) {
        ...
        return Math.min(getScaleLimit(b), s);
    }
    ...
    private float getMaximalScale(Box b) {
        ...
        return getScaleLimit(b);
    }

    // 根据View和Image的宽高动态计算缩放限制
    private float getScaleLimit(Box b){
        if (b.mUseViewSize){
            return SCALE_LIMIT;
        }
        if (b.mImageW >= mViewW * 2 || b.mImageH >= mViewH * 2) {
            return 1f; // 如果图片宽度或高度大于视窗的两倍，图片最大缩放比为1
        }
        if (b.mImageW >= mViewW || b.mImageH >= mViewH) {
            return 2f; // 如果图片宽度或高度大于视窗，图片最大缩放比为2
        }
        if (b.mImageW >= mViewW * 0.5 || b.mImageH >= mViewH * 0.5) {
            return 3f; // 如果图片宽度或高度大于视窗的一半，图片最大缩放比为3
        }
        return SCALE_LIMIT; // 其它情况采用现有的缩放比5
    }
