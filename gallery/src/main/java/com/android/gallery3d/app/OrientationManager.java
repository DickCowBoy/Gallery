...
public class OrientationManager implements OrientationSource {
    ...
    // 是否允许使用系统旋转设置,默认为false
    private boolean mEnableUsingSystemOrientation = false;
    ...
    // 纵向锁屏
    public void lockOrientationWithPortrait() {
        unlockOrientation();
    ...
    // 获取系统设置锁屏
    public void setupSystemSettingLocking() {
    ...
    // 允许使用系统设置
    public void enableUsingSystemOrientation() {
        mEnableUsingSystemOrientation = true;
    }

    // 禁止使用系统设置
    public void diableUsingSystemOrientation() {
        mEnableUsingSystemOrientation = false;
    }
    ...
    private class MyOrientationEventListener extends OrientationEventListener {
        ...
        public void onOrientationChanged(int orientation) {
            ...
            // 如果不允许系统设置,则直接设置竖屏并返回
            if (!mEnableUsingSystemOrientation) {
                mActivity.setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_PORTRAIT);
                return;
            }
            // 获取系统设置是否锁屏,原因是有可能是在当前页面下拉通知栏变更旋转屏幕设置
            mRotationLockedSetting = Settings.System.getInt(mActivity.getContentResolver(),
                    Settings.System.ACCELEROMETER_ROTATION, 0) != 1;
            if (mRotationLockedSetting) {
                mActivity.setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_PORTRAIT);
                return;
            }
            // 设置使用方向传感器监听方向,这样能够保证旋转不出现旋转方向抖动的情况
            mActivity.setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_SENSOR);
        }
    }
