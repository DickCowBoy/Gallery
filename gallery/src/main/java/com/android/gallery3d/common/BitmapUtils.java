...
public class BitmapUtils {
    ...
    public static Bitmap createVideoThumbnail(ContentResolver cr, int videoId, String filePath) {
        Bitmap videoBmp = createVideoThumbnail(cr, videoId);
        if (videoBmp != null) {
            return videoBmp;
        }
    ...
    // 使用系统媒体库生成的缓存
    public static Bitmap createVideoThumbnail(ContentResolver cr, int videoId) {
        try {
            BitmapFactory.Options options = new BitmapFactory.Options();
            options.inPreferredConfig = Bitmap.Config.ARGB_8888;
            return MediaStore.Video.Thumbnails.getThumbnail(cr, videoId, MediaStore.Images.Thumbnails.MINI_KIND, options);
        } catch (Exception e) {
    ...
    public static boolean isCropSupported(String mimeType) {
        if (mimeType == null) return false;
        mimeType = mimeType.toLowerCase();

        // supported if image type is NOT gif
        return mimeType.startsWith("image/") &&
                !mimeType.equals("image/gif");
    }

    public static boolean isPlaySupported(String mimeType) {
        if (mimeType == null) return false;
        mimeType = mimeType.toLowerCase();

        // support play if image type is gif
